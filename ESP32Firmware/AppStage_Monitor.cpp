...
bool AppStage_Monitor::onCommand(const std::vector<std::string>& args, WSCommandResponse& results)
{
...
void AppStage_Monitor::pause()
{
  ...
  WebSocketManager* wsManager = WebSocketManager::getInstance();
  wsManager->setControlEnabled(false);
  wsManager->clearCommandHandler(this);
  Serial.println("DISABLE WebSocket command processing");
}
